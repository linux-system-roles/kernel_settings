...
- ci: use commit hash for github action, add persist-credentials false [citest_skip] (#327)
- ci: refactor build_docs so that pandoc runs in isolated read-only job [citest_skip] (#328)
- ci: use exact match for systemroller instead of contains [citest_skip] (#329)
- ci(build_docs): fix pandoc container syntax [citest_skip] (#330)
- refactor: Relax collection constraints, gate vendored modules by python version, update ci versions [citest_skip] (#336)
- ci: do not run ci tests by default, require citest comment or label [citest_skip] (#338)
- ci: replace weekly_ci with periodic_ci, stagger schedules [citest_skip] (#339)
...
- ci: use gha checkout v7, codecov v7 [citest_skip] (#316)
- ci: Use our own pr_title_lint.py instead of NPM commitlint [citest_skip] (#317)
- ci: bump tox-lsr version to 3.20.0 to fix tox 4.58 api breakage [citest_skip] (#318)
- ci: Add support for Fedora 44 and drop Fedora 42 - use ansible-core 2.21 [citest_skip] (#320)
- ci: bump github/codeql-action from 4 to 4.37.3 (#321)
- ci: bump actions/setup-python from 6 to 7 (#322)
- ci: ensure dependabot updates do not invoke ci tests [citest_skip] (#323)
- ci: [citest_skip] bump github/codeql-action from 4.37.3 to 4.37.4 (#324)
...
- ci: bump actions/github-script from 8 to 9 (#310)
- refactor: use ansible.posix 2.1.X for EL7 compatibility [citest_skip] (#311)
- ci: Add config file for CodeRabbit with custom rules (#312)
- ci: Skip reviews for PRs with [citest_skip] in the title (#313)
- test: remove unused files [citest_skip] (#314)
...
- ci: use ANSIBLE_INJECT_FACT_VARS=false by default for testing (#288)
- ci: bump ansible/ansible-lint from 25 to 26 (#289)
- ci: skip most CI checks if title contains citest skip [citest_skip] (#290)
- ci: ansible-lint - remove .collection directory from converted collection [citest_skip] (#291)
- ci: tox-lsr version 3.15.0 [citest_skip] (#292)
- ci: Add Fedora 43, remove Fedora 41 from Testing Farm CI (#293)
- ci: bump actions/upload-artifact from 6 to 7 (#296)
- ci: tox-lsr 3.17.0 - container test improvements, use ansible 2.20 for fedora 43 [citest_skip] (#298)
- ci: tox-lsr 3.17.1 - previous update broke container tests, this fixes them [citest_skip] (#299)
- ci: fix yum repos to use devel site instead of old site name [citest_skip] (#301)
- ci: use codecov @v6 [citest_skip] (#302)
- refactor: comply with Ansible partner certification checks (#303)
- ci: update header for run_role_with_clear_facts [citest_skip] (#304)
- test: support ansible-test milestone version 2.22 [citest_skip] (#305)
- ci: Comply with Ansible partner certification checking [citest_skip] (#306)
- ci: ansible-lint requires dependencies to be installed [citest_skip] (#307)
